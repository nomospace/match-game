...
define(function(require, exports) {
     ...
     * @class   连连看主模块类
     */
    var MatchGameModule = Class();
    var protoModule = MatchGameModule.prototype;
    /**
     * 默认初始化函数
     * @return {Void}
     */
    protoModule.initialize = function() {
        this._intXnode();
        this._intModule();
    };
    /**
     ...
     * @return {Void}
     */
    protoModule._intXnode = function() {
     ...
     * @return {Void}
     */
    protoModule._intModule = function() {
        this.defaultLineCount = 8;
        this._defaultColumnCount = 8;
        this.__defaultTotalCount = this.defaultLineCount * this._defaultColumnCount;
        this._startGame();
    };
    /**
     * 开始游戏
     * @return {Void}
     */
    protoModule._startGame = function() {
        this._matrix = this._getDefaultMatrix();
        for (var i = 0, l = this.totalCount.value; i < l; i++) {
     ...
     * @return {Array}
     */
    protoModule._getMatchData = function() {
     ...
     * @return {Array}
     */
    protoModule._getDefaultMatrix = function() {
        //	临时二位数组
        for (var i = 0, x = this.defaultLineCount, _matrix = []; i < x; i++) {
            _matrix[i] = [];
            for (var j = 0, y = this._defaultColumnCount; j < y; j++) {
                _matrix[i][j] = {};
            }
        }
        return _matrix;
     ...
     * @return {Void}
     */
    protoModule._draw = function() {
        var _class = MatchGamePoint;
        this._matchGamePoints && _class.recycle(this._matchGamePoints);
        this._matchGamePoints =
            _class.allocate(this.matchGameData, this.container, {onselect: this.__onPointSelected.bind(this) || F});
    };
    /**
     * 随机生成两两配对的矩阵
     ...
     * @return {Void}
     */
    protoModule._initMatrixData = function(_value, _color, _done) {
        var _position = this._getRandomPosition(), _value = _value || this._getRandomImageValue(), _color = _color || this._getRandomImageColor();
     ...
     * @param {Object} _color    元素样式值
     */
    protoModule._setPointValue = function(_position, _value, _color) {
        this._matrix[_position.x][_position.y] = {
            ...
            position: {
                x: _position.x,
                y: _position.y,
                index: _position.x * this._defaultColumnCount + _position.y
            }
        };
    };
    /**
     ...
     * @return {Object}
     */
    protoModule._getRandomPosition = function() {
        return {
            x: Math.floor(Math.random() * this.defaultLineCount),
            y: Math.floor(Math.random() * this._defaultColumnCount)
        };
    };
    /**
     ...
     * @return {String}    元素值
     */
    protoModule._getRandomImageValue = function() {
...
//			(Math.floor(Math.random() * 127)));
     ...
     */
    protoModule._getRandomImageColor = function() {
        return Math.floor(0xffff00 * Math.floor(Math.random() * this.imageTypeCount.value + 1) / this.totalCount.value).toString(16);
    };
    /**
     * 选择格子
     * @param {Object} _data
     ...
     */
    protoModule.__onPointSelected = function(_data, _selected) {
        for (var i = 0, l = this._matchGamePoints.length, _item, _selectedPoints = []; i < l; i++) {
            _item = this._matchGamePoints[i];
            if (_item.isSelected()) {
     ...
     * @return {Void}
     */
    protoModule._checkMatch = function(_points) {
        if (!$.isArray(_points) || _points.length != 2) {
            this._draw();
            return null;
        }
        var _pointA = _points[0], _pointB = _points[1], _dataA = _pointA.getData(), _dataB = _pointB.getData(), _positionA = _dataA.position, _positionB = _dataB.position;
...
//                }
     ...
     * @return {Boolean}
     */
    protoModule._checkValue = function(_dataA, _dataB) {
     ...
     */
//	protoModule.__isNeighbour = function(a, b, _distance){
...
//	};
     ...
     * @param {Object} b
     */
    protoModule._checkHorizon = function(a, b) {
     ...
     * @param {Object} b
     */
    protoModule._checkVertical = function(a, b) {
     ...
     * @param {Object} b
     */
    protoModule._hasOneCorner = function(a, b) {
     ...
     * @param {Object} b
     */
    protoModule._hasTwoCorner = function(a, b) {
        ...
        for (var index = 0; index < _tmpList.length; index++) {
            ...
            if (this._checkHorizon(a, _line.a) && this._checkHorizon(b, _line.b))
                return true;
        }
        return false;
    };
     ...
     * @param {Object} b
     */
    protoModule._scanTheRoute = function(a, b) {
        ...
        for (var x = a.x; x >= 0; x--)
            _function();
        for (var x = a.x; x < this._defaultColumnCount; x++)
            _function();
     ...
     * @param {Object} b
     */
    protoModule._getPoint = function(a, b) {
        return {x:a,y:b};
    };
    /**
     * 设置坐标
     ...
     * @param {Object} b
     */
    protoModule._setLine = function(_direct, a, b) {
        //	todo:绘制最短路径
     ...
     * @return {Void}
     */
    protoModule._removeMatrix = function(a, b) {
     ...
     * @return {Boolean}
     */
    protoModule._checkDeadlock = function() {
    };
    /**
     * 连连看单个格子项对象
    ...
    protoPoint.reset = function(_options) {
        this.addEvent('onselect', _options.onselect || F);
    };
    /**
     * 初始化节点
     * @return {Void}
    ...
    protoPoint._initXnode = function() {
        this.point = this.body.children(0);
    ...
    protoPoint.toggle = function(_elm, _class) {
        _elm.has(_class) ? _elm.removeClass(_class) : _elm.addClass(_class)
    };
    /**
     * 返回格子项当前的选择状态
     * @return {Boolean}
